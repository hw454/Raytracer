...
# --- These will be functions to be performed on SM's and DSM's
...
# ------------ These tests test the creation of the dictionary sparse matrix
...
# ----------- These tests, test matrix operations on matrices from DSM
...
def test_08():
  ...
  indices=zip(*M0.nonzero())
  M1= SM(M0.shape,dtype=np.float)
  M1[iindices]=np.cos(np.angle(M0[indices].todense()))
  print(M0,M1)
...
def test_09():
  ...
  M1[indices]=np.cos(np.angle(M0[indices].todense()))
  M2[indices]=np.cos(0.7*np.angle(M0[indices].todense()))
...
# ----- Test using array operations on the DSM
