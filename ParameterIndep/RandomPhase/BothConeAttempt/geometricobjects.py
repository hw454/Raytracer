  ...
  def uniform_ray_tracer(s,origin,n,ave,i,frequency,start,m,refloss):
    ...
    for it in range(0,n+1):
      ...
      #for k in range(1,ave):
        #Mesh2=ray.heatmaprayrndboth(Mesh2,ray.streg,ray.frequency,spacing,refloss,n)
    ...
    mp.savefig('../../../../NewFigures/ConeNoPhaseHeatmap'+str(i)+'.png',bbox_inches='tight')
    #mp.figure(i+2)
    ##s.Plotroom(origin)
    #cbar=mp.colorbar()
    #cbar.set_label('Power in dBm', rotation=270)
    ...
    cbar=mp.colorbar()
    cbar.set_label('Power in dBm', rotation=270)
    mp.savefig('../../../../NewFigures/ConeAveragedHeatmapwithPhase'+str(i)+'.png',bbox_inches='tight')
    c0=Mesh0.hist(i+5)
    mp.figure(i+5)
