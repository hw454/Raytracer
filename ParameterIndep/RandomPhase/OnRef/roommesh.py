...
class roommesh:
  ...
  def hist(s,i):
     ...
     mp.grid()
     mp.savefig('../../../../ImagesOfSignalStrength/FiguresNew/RandomPhase/OnRefRNDHistogramNoBounds'+str(i)+'.png',bbox_inches='tight')
     return
  def histbounded(s,i):
     z=np.absolute(s.grid)
     z=10*np.ma.log10(z)
     ...
     mp.grid()
     mp.savefig('../../../../ImagesOfSignalStrength/FiguresNew/RandomPhase/OnRefRNDCumsumBounded'+str(i)+'.png',bbox_inches='tight')
     mp.figure(i+1)
     mp.hist(z.flatten(),bins='auto')
     #mp.plot(h[1][:-1],h[0]) Plots the histogram as a line
     mp.title('Histrogram of bounded signal power')
     mp.grid()
     mp.savefig('../../../../ImagesOfSignalStrength/FiguresNew/RandomPhase/OnRefRNDHistogramBounds'+str(i)+'.png',bbox_inches='tight')
     return
  def teststrength(s):
