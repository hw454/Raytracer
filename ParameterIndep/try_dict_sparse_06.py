...
def sparse_angles(M):
  ...
  return AngM,t2-t1,t3-t2

def sparse_angles_diff(M):
  '''Takes in a complex sparse matrix M and outputs the arguments of the nonzero() entries'''
  AngM=SM(M.shape,dtype=float)
  AngM2=SM(M.shape,dtype=float)
  indices=M.nonzero()
  t1=time.time()
  AngM2[indices]=np.arctan2(np.imag(M[indices]),(np.real(M[indices])))
  t2=time.time()
  #print('first time', t2-t1)
  #print('second time',t3-t2)
  #print('Difference in times', t2-t1-t3+t2)
...
def test_03c(nx,ny,nz,na,nb):
  ...
  for x,y,z,a in product(range(nx),range(ny),range(nz),range(na)):
    if a<nb:
      for ai in range(a-1,a+1):
        ds[x,y,z,ai-1,ai-1]=complex(a,a)
  return ds
...
def test_11():
  '''Extract reflection angles from matrix - timing test'''
  nx=1
  ny=1
  nz=1
  nt=8
  ...
  for l in range(nt):
    for i,j,k in product(range(nx),range(ny),range(nz)):
      M=DSM[i,j,k][0:na,0:nb]
      AngM,t1[l],t2[l]=sparse_angles(M)
      AngM, t3[l]=sparse_angles_diff(M)
    n[l]=na*nb
    na=int(na/2)
    nb=int(nb/2)
  times=np.append([n],[t1],axis=0)
  times=np.append(times,[t2],axis=0)
  times=np.append(times,[t3],axis=0)
