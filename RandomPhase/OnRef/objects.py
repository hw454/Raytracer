...
class room:
  ...
  def uniform_ray_tracer_bounded(s,origin,n,i,spacing,frequency,start,m,bounds,refloss):
    ...
    #mp.figure(i+3)
    #s.Plotrom(origin)
    #mp.savefig('../../../../ImagesOfSignalStrength/FiguresNew/RandomPhase/OnReftemproom.png',bbox_inches='tight')#This section is the same as the unbounded
    #s.Plotroom(origin)
    mp.figure(i+1)
    mp.title('Heatmap Bounded')
    mp.savefig('../../../../ImagesOfSignalStrength/FiguresNew/RandomPhase/OnRefHeatmapBounded'+str(i)+'.png',bbox_inches='tight')
    s.Plotroom(origin)
