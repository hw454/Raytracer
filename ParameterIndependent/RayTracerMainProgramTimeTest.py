...
def MeshProgram(Nr=22,index=1,job=0,Nre=3,PerfRef=0,LOS=0,InnerOb=0,Nrs=2,Ns=5,delangle=np.pi/4):
  ...
  * 'Directions.npy' - An Nrax3x1 array containing the vectors which \
  ...
  meshname=meshfolder+'/TimeTestDSM_tx%03d'%(job)
  ...
  print('Tx',Tx)
  programterms=np.array([Nr,Nre,AngChan,split,splitinv,delangle])
...
def StdProgram(Nr=22,index=1,job=0,Nre=3,PerfRef=0,LOS=0,InnerOb=0,Nrs=1,Ns=5,delangle=np.pi/4):
...
def power_grid(Room,Mesh,Nr=22,index=0,job=0,Nre=3,PerfRef=0,LOS=0,InnerOb=0,Nrs=2,Ns=5,delangle=np.pi/4):
...
def main(argv,scriptcall=False):
  ...
  if not os.path.exists('./Times'):
    os.makedirs('./Times')
  parameters=  np.load('Parameters/ParameterarrayTimeTest.npy')
  _,_,L,split    =np.load('Parameters/Raytracing.npy')
  Nra= np.load('Parameters/NraFull.npy')
  nra=len(Nra)
  nre=5 #Starts at 2
  ns=5 #Starts at 5
  delangle=np.load('Parameters/delangleFull.npy')
  Ttest=parameters.shape[0]
  TimeMat=np.zeros((nra,nre,ns,Ttest,8))
  for j,arr in enumerate(parameters):
    InnerOb,Nrind,Nrs,LOS,Nre,PerfRef,Ns,Q,Par,index=arr.astype(int)
    #Tx=np.load('Parameters/Origin.npy')
    ##----Retrieve the environment--------------------------------------
    Nr=Nra[Nrind]
    delth=delangle[Nrind]
    h=1.0/Ns
    ##----The lengths are non-dimensionalised---------------------------
    Oblist     =np.load('Parameters/Obstacles%d.npy'%index).astype(float)      # The obstacles which are within the outerboundary
    Ntri       =np.load('Parameters/NtriOut.npy')                      # Number of triangles forming the surfaces of the outerboundary
    ...
    if scriptcall:
      ...
      #Tx=MoveTx(job,Nx,Ny,Nz,h)
      np.save('Parameters/Origin_job%03d.npy'%job,Tx)
    tstd0=t.process_time()
    Pstd,tstd=StdProgram(Nr,index,job,Nre,PerfRef,LOS,InnerOb,Nrs,Ns,delth)
    tstd=t.process_time()-tstd0
    print('-------------------------------')
    print('Time for std program')
    print(tstd)
    print('-------------------------------')
    timemesh0=t.process_time()
    Mesh1,timemesh,Room=MeshProgram(Nr,index,job,Nre,PerfRef,LOS,InnerOb,Nrs,Ns,delth)# Shoot the rays and store the information
    timemesh=t.process_time()-timemesh0
    timep0=t.process_time()
    ...
    print(timemesh,timep,timeo)
    print('-------------------------------')
    TimeMat[Nrind,Nre-2,0,7]+=1
    ic=TimeMat[Nrind,Nre-2,0,7].astype(int)
    TimeMat[Nrind,Nre-2,ic,0]=tstd
    TimeMat[Nrind,Nre-2,ic,2]=timemesh
    TimeMat[Nrind,Nre-2,ic,3]=timep
    TimeMat[Nrind,Nre-2,ic,4]=timeo
    TimeMat[Nrind,Nre-2,ic,5]=timemesh+timep
    #if ic>0:
      #TimeMat[Nrind,Nre-2,0,1]=sum(TimeMat[Nrind,Nre-2,0:(ic+1),0])/ic
      #TimeMat[Nrind,Nre-2,0,6]=sum(TimeMat[Nrind,Nre-2,0:(ic+1),5])/ic
    print('-------------------------------')
    print('-------------------------------')
  np.save('Times/TimeStd.npy',TimeMat[:,:,:,0:2])
  np.save('Times/TimeGRL.npy',TimeMat[:,:,:,2:])
